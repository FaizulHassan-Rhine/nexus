...
export default function ResearcherSupportPage() {
  ...
  const thread = active?.conversation || active?.replies?.map((r) => ({ at: r.at, by: r.authorId, body: r.body, type: "public" })) || [];
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Support"
        description="Help with grant applications, dataset publishing, technology transfer, and collaborations"
          ...
          {active ? (
            <div className="flex h-full flex-col">
              <div className="border-b border-slate-200 p-4 dark:border-slate-700">
                ...
                <p className="text-sm text-secondary">{active.category} · {active.priority} · Opened {formatDate(active.createdAt)}</p>
                ...
                {thread.map((m, i) => {
                  ...
                  return (
                    <div key={i} className="rounded-lg bg-slate-50 p-3 text-sm dark:bg-slate-800">
                      <p className="text-xs font-medium">{author?.name || m.by}</p>
              ...
              {active.status !== "Resolved" ? (
                <div className="flex gap-2 border-t border-slate-200 p-3 dark:border-slate-700">
                  <Input className="flex-1" placeholder="Reply..." value={reply} onChange={(e) => setReply(e.target.value)} />
      ...
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="Create support ticket">
        <div className="space-y-4">
          ...
          <Select
            ...
            options={["Technical issue", "Grant support", "Dataset publishing", "Technology transfer", "Collaboration", "General inquiry"]}
          />
          <Select
            label="Priority"
            value={form.priority}
