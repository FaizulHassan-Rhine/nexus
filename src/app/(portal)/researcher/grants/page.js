...
export default function ResearcherGrantsPage() {
  ...
  const grantApps = useMemo(() => {
    const mine = user ? getResearcherApplications(applications, user.id) : [];
  ...
  const checkEligibility = (opp) => {
    ...
    toast.message(deptOk ? "Eligible based on department and researcher profile" : "Department mismatch — review requirements");
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Grants" description="Grant opportunities, proposal drafts, budget planning, and application status" />
      ...
      <Tabs defaultValue="calls">
        <TabList>
          ...
          <Tab value="status">Applications</Tab>
        </TabList>
        ...
        <TabPanel value="draft">
          <div className="card-surface max-w-2xl space-y-4 p-4">
            ...
            <Textarea
              ...
              placeholder="Co-investigators, research assistants..."
            />
        ...
        <TabPanel value="status">
          {grantApps.length ? (
            grantApps.map((app) => {
              const opp = grantOpps.find((o) => o.id === app.opportunityId) || opportunities.find((o) => o.id === app.opportunityId);
              return (
