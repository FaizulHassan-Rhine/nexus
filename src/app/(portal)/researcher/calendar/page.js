...
import {
  ...
  researcherCalendarEvents,
} from "../_lib/helpers";

export default function ResearcherCalendarPage() {
  ...
  const events = useMemo(() => {
    const userApps = user ? getResearcherApplications(applications, user.id) : [];
    const userProjects = user ? getResearcherProjects(projects, user.id, user) : [];
    return researcherCalendarEvents(user, userApps, opportunities, userProjects);
  }, [user, applications, opportunities, projects]);
  ...
  const downloadEvent = (event) => {
    downloadIcs({
      title: event.title,
      description: `${event.type} — Nexus researcher calendar`,
      start: event.date,
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Calendar"
        description="Grant deadlines, collaboration milestones, interviews, and project dates"
        actions={
          <Button
            variant="secondary"
            onClick={() => {
      ...
      <Tabs defaultValue="month">
        ...
        <TabPanel value="list">
          <ul className="space-y-2">
            ...
            {!events.length && <p className="text-secondary">No calendar events yet.</p>}
          </ul>
        </TabPanel>
      </Tabs>
    </div>
