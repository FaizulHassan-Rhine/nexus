...
export default function FacultyCalendarPage() {
  ...
  const events = useMemo(() => {
    ...
    const userProjects = user ? getFacultyProjects(projects, user.id) : [];
    return facultyCalendarEvents(user, userApps, opportunities, userProjects);
  ...
  const downloadEvent = (event) => {
    downloadIcs({
      title: event.title,
      description: `${event.type} — Nexus faculty calendar`,
      start: event.date,
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Calendar"
        description="Exchange deadlines, grant dates, mentoring sessions, and project milestones"
        actions={
          <Button
            variant="secondary"
            onClick={() => {
